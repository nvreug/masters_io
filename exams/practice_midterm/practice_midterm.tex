...
\begin{parts}
	...
	\part[3] In the logit demand model, write the formula for the own-price elasticity $\eta_{jj}$ in terms of the price coefficient $\alpha$, price $p_j$, and market share $s_{jt}$.

	\answerline[$\eta_{jj} = \alpha p_{jt} (1 - s_{jt})$]
...
where $\delta_{jt}$ is the mean utility (excluding price), $\alpha = -0.5$ is the price coefficient, and $\varepsilon_{ijt}$ is i.i.d. Type 1 Extreme Value.

The following data are observed:
\begin{center}
\begin{tabular}{lccc}
\toprule
Product & Price ($p_j$) & Mean Utility ($\delta_{jt}$) & Market Share ($s_{jt}$) \\
\midrule
1 & \$10 & 2.0 & 0.25 \\
2 & \$8  & 1.5 & 0.20 \\
3 & \$12 & 2.5 & 0.15 \\
...
\begin{parts}
	...
	$$s_{jt} = \frac{\exp(\delta_{jt} + \alpha p_{jt})}{1 + \sum_k \exp(\delta_{kt} + \alpha p_{kt})}$$
	...
	\part[5] Suppose you only observe prices and market shares (not $\delta_{jt}$). Write down the Berry inversion formula that would allow you to recover $\delta_{jt} + \alpha p_{jt}$ from the data.
	...
	$$CS = \frac{1}{|\alpha|} \ln\left(1 + \sum_j \exp(\delta_{jt} + \alpha p_{jt})\right)$$
