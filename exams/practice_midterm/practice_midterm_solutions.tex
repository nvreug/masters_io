...
\begin{parts}
	...
	\begin{solution}
	$\boxed{\eta_{jj} = \alpha p_{jt} (1 - s_{jt})}$
	\end{solution}

	\part[3] True/False: Higher prices = more elastic demand in logit.
	\begin{solution}
	\textbf{True.} From the formula $\eta_{jj} = \alpha p_{jt} (1 - s_{jt})$, since $\alpha < 0$ and $(1-s_{jt}) \approx 1$ for small shares, elasticity is roughly proportional to price. This is a mechanical relationship imposed by the functional form.
	\end{solution}
...
\begin{parts}
	...
	\begin{solution}
	From logit: $\frac{s_{jt}}{s_{0t}} = \exp(\delta_{jt} + \alpha p_{jt})$

	Taking logs: $\ln(s_{jt}) - \ln(s_{0t}) = \delta_{jt} + \alpha p_{jt}$

	\textbf{Berry inversion:} $\boxed{\delta_{jt} + \alpha p_{jt} = \ln(s_{jt}) - \ln(s_{0t})}$
	...
	\begin{solution}
	Using $v_j = \delta_{jt} + \alpha p_{jt}$:
