...
\begin{enumerate}
\item \textbf{(5 points)} Solve for the pre-merger equilibrium prices. Each firm owns one product and sets price to maximize profit. Use the first-order condition:
\[
s_j + (p_j - c_j) \frac{\partial s_j}{\partial p_j} = 0
\]
Solve this system numerically in Python starting from initial guess $p = c + 0.5$.

\item \textbf{(5 points)} Using your equilibrium prices, compute market shares for each product and the outside option.
...
\item \textbf{(5 points)} Verify that your prices satisfy the FOC by checking that:
\[
...
\frac{\partial s_{jt}}{\partial p_{kt}} &= -\alpha s_{jt} s_{kt} \quad (j \neq k)
\end{align*}
...
\begin{align*}
CS_t = \frac{1}{|\alpha|} \ln\left(1 + \sum_j \exp(\delta_{jt} + \alpha p_{jt})\right)
\end{align*}

\textbf{HHI:}
\begin{align*}
HHI_t = \sum_j (100 \times s_{jt})^2
\end{align*}
