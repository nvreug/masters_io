...
The Berry inversion starts from the logit choice probability. For product $j$, the market share is $s_{jt} = \exp(\delta_{jt}) / (1 + \sum_{k=1}^{J} \exp(\delta_{kt}))$, while the outside option share is $s_{0t} = 1 / (1 + \sum_{k=1}^{J} \exp(\delta_{kt}))$. Taking the ratio gives $s_{jt} / s_{0t} = \exp(\delta_{jt})$, and applying logs yields $\ln(s_{jt}) - \ln(s_{0t}) = \delta_{jt}$. Substituting the expression for mean utility $\delta_{jt} = \beta_1 \cdot \text{sugar}_{jt} + \alpha \cdot p_{jt} + \xi_{jt}$, we obtain the estimating equation:
\begin{align*}
\boxed{\ln(s_{jt}) - \ln(s_{0t}) = \beta_1 \cdot \text{sugar}_{jt} + \alpha \cdot p_{jt} + \xi_{jt}}
\end{align*}

This is a linear regression (without intercept) where the dependent variable is the log difference between product share and outside good share, and the error term is $\xi_{jt}$.
...
\midrule
$\hat{\beta}_1$ (sugar) & $0.0389$ \\
$\hat{\alpha}$ (price) & $-32.1018$ \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
$\hat{\beta}_1$ (sugar) & $0.0285$ \\
$\hat{\alpha}$ (price) & $-32.1253$ \\
\bottomrule
\end{tabular}
\end{center}
...
The OLS estimate of the price coefficient is slightly less negative than the 2SLS estimate ($-32.10$ vs $-32.13$). This difference arises because prices are likely correlated with unobserved product quality. Firms set higher prices for products that consumers value for reasons not captured by sugar content, such as brand reputation or taste. This positive correlation between price and the error term biases the OLS estimate of $\alpha$ toward zero. The 2SLS estimate, which uses instruments that are correlated with price but uncorrelated with unobserved quality, produces a more negative coefficient indicating that consumers are more price sensitive than OLS suggests.
...
The term $\delta_{jt}$ is the mean utility of product $j$ in market $t$, representing the portion of utility that is common to all consumers. In this model, $\delta_{jt} = \beta_1 \cdot \text{sugar}_{jt} + \alpha_0 \cdot p_{jt} + \xi_{jt}$. It captures the average valuation of the product before accounting for individual-specific variation from demographics or the idiosyncratic logit shock.
...
$\hat{\beta}_1$ (sugar) & $0.0718$ \\
$\hat{\alpha}_0$ (price) & $-40.7163$ \\
\midrule
\multicolumn{2}{l}{\textit{Demographic interactions}} \\
$\hat{\beta}_{1,inc}$ (income $\times$ sugar) & $-0.0498$ \\
$\hat{\alpha}_{inc}$ (income $\times$ price) & $32.9185$ \\
\bottomrule
\end{tabular}
\end{center}
...
The parameter $\alpha_{inc}$ captures how price sensitivity varies with income. Since $\alpha_{inc} > 0$, higher-income consumers are less sensitive to price---their effective price coefficient $(\alpha_0 + \alpha_{inc} \cdot \text{income})$ is less negative than for lower-income consumers.
