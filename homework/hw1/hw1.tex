...
\item The data files are available on Canvas.
\item \textbf{AI policy:} You may use AI tools (e.g., ChatGPT, Claude) as a reference, but you may \textit{not} use AI to directly answer the questions. 
\end{itemize}
...
\begin{align*}
	u_{ijt} = \beta_1 \cdot \text{sugar}_{jt} + \alpha \cdot p_{jt} + \xi_{jt} + \epsilon_{ijt}
\end{align*}
where:
\begin{itemize}
...
\textbf{Important:} The market shares in the data will \textit{not} sum to 1 within each market. The residual share ($1 - \sum_j s_{jt}$) is the outside option share $s_{0t}$.
...
\begin{enumerate}
	...
	\item \textbf{(5 points)} Estimate the model using OLS. Report the coefficients $\hat{\beta}_1$ and $\hat{\alpha}$. (No need to report standard errors.)
...
\begin{align*}
	u_{ijt} = (\beta_1 + \pi_1 \cdot \text{income}_i) \cdot \text{sugar}_{jt} + (\alpha + \pi_\alpha \cdot \text{income}_i) \cdot p_{jt} + \xi_{jt} + \epsilon_{ijt}
\end{align*}

This model has four parameters:
\begin{itemize}
	\item \textbf{Linear parameters:} $\beta_1$ (sugar), $\alpha$ (price)
	\item \textbf{Demographic interactions (pi):} $\pi_1$ (income $\times$ sugar), $\pi_\alpha$ (income $\times$ price)
\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{(20 points)} Estimate this model using \texttt{pyblp}. Report all four parameter estimates. (No need to report standard errors.)

	\textit{Hint:} Use \texttt{sigma=0} (no random coefficients) and the following starting values for \texttt{pi}:
\begin{verbatim}
initial_pi = np.array([
    [0.1],   # income effect on sugar
    [-5.0]   # income effect on price
])
\end{verbatim}

	\item \textbf{(10 points)} Interpret the parameter $\pi_\alpha$. (2 sentences max.)
