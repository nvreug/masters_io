...
\begin{frame}{Plan}
  \begin{wideenumerate}
    \item \textbf{Review: Part 1 (Demand and Pricing)}
...
\begin{frame}{Demand estimation: key concepts}
	\begin{wideitemize}
		\item \textbf{Logit model:} $s_{jt} = \frac{\exp(\delta_{jt})}{1 + \sum_k \exp(\delta_{kt})}$
		\item \textbf{Berry inversion:} $\ln(s_{jt}) - \ln(s_{0t}) = \delta_{jt}$
		\item \textbf{Elasticities:}
		\begin{align*}
			\eta_{jj} = \alpha p_{jt} (1 - s_{jt}) \quad \text{(own)} \\
			\eta_{jk} = -\alpha p_{kt} s_{kt} \quad \text{(cross)}
		\end{align*}
		\item \textbf{Consumer surplus:} $CS = \frac{1}{|\alpha|} \ln\left[\sum_j \exp(\delta_{jt})\right]$
...
\begin{frame}{T/F: Demand estimation}
	...
	\begin{wideenumerate}
		...
		\item Consumer surplus from logit depends on the log-sum of $\exp(\delta_{jt})$.
	\end{wideenumerate}
	\vspace{8pt}
	\centering
...
\begin{frame}{T/F: Demand estimation (solutions)}
	\begin{wideenumerate}
		...
		\item \textbf{TRUE.} $CS = \frac{1}{|\alpha|} \ln\left[\sum_j \exp(\delta_{jt})\right]$ (the log-sum formula).
	\end{wideenumerate}
\end{frame}

\begin{frame}{Identification and IVs}
	\begin{wideitemize}
		\item \textbf{Problem:} $E[p_{jt} \xi_{jt}] \neq 0$ (price endogeneity)
...
\begin{frame}{Plan}
  \begin{wideenumerate}
    ...
    \item \textbf{Practice problems: Part 1}
    \item Review: Part 2 (Competition)
    \item Practice problems: Part 2
    \item Exam logistics
  \end{wideenumerate}
...
\begin{frame}{Practice: Consumer surplus (solution)}
	\begin{wideitemize}
		\item \textbf{(a)} $CS = \frac{1}{|\alpha|} \ln\left[1 + \sum_j \exp(\delta_{jt})\right]$
		\begin{wideitemize}
			\vspace{3pt}
...
\begin{frame}{Plan}
  \begin{wideenumerate}
    \item Review: Part 1 (Demand and Pricing)
    \item Practice problems: Part 1
    \item \textbf{Review: Part 2 (Competition)}
    \item Practice problems: Part 2
    \item Exam logistics
  \end{wideenumerate}
\end{frame}
...
\begin{frame}{Plan}
  \begin{wideenumerate}
    ...
    \item Practice problems: Part 1
    \item Review: Part 2 (Competition)
    \item \textbf{Practice problems: Part 2}
    \item Exam logistics
  \end{wideenumerate}
\end{frame}
...
\begin{frame}{Comprehensive summary table}
	\begin{center}
		...
		\begin{tabular}{|p{2.5cm}|p{4.5cm}|p{4.5cm}|}
			...
			\hline
			Logit & $s_{jt} = \exp(\delta_{jt})/[1+\Sigma\exp(\delta_{kt})]$ & IIA: proportional substitution \\
			\hline
			Elasticities & $\eta_{jj} = \alpha p_{jt}(1-s_{jt})$ & More elastic $\rightarrow$ lower markup \\
			\hline
...
\begin{frame}{Key formulas for your cheat sheet}
	\begin{wideitemize}
		\item Logit: $s_{jt} = \exp(\delta_{jt})/[1 + \Sigma\exp(\delta_{kt})]$
		\item Berry: $\ln(s_{jt}) - \ln(s_{0t}) = \delta_{jt}$
		\item Elasticities: $\eta_{jj} = \alpha p_{jt}(1-s_{jt})$, $\eta_{jk} = -\alpha p_{kt} s_{kt}$
