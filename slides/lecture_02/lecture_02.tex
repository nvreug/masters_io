...
\begin{frame}{Plan for today}
  \begin{wideenumerate}
    \item \textbf{Logit model derivation}
...
\begin{frame}{Plan for today}
  \begin{wideenumerate}
    \item Logit model derivation
    \item Berry (1994) inversion
    \item \textbf{Elasticity formulas}
    \item IIA problem (preview)
    \item[] \rule{0.5\textwidth}{0.4pt}
    \item The identification problem
    \item Direction of bias
...
\begin{frame}{Worked example: Berry inversion}
	\begin{wideitemize}
		...
		\begin{align*}
			s_0 = 0.5, \quad s_1 = 0.3, \quad s_2 = 0.2
		\end{align*}
...
\begin{frame}{Worked example: Berry inversion (solution)}
	\begin{solutionbox}[Solution]
		\begin{itemize}
			\item Berry inversion: $\ln(s_j) - \ln(s_0) = \delta_j$
			\item For product 1:
			\begin{align*}
...
\begin{frame}{Plan for today}
  \begin{wideenumerate}
    ...
    \item Berry (1994) inversion
    \item Elasticity formulas
    \item \textbf{IIA problem (preview)}
    \item[] \rule{0.5\textwidth}{0.4pt}
    \item The identification problem
    \item Direction of bias
    \item Instrumental variables
...
\begin{frame}{What drives the IIA property?}
	\begin{wideitemize}
		\item IIA comes from the \textbf{logit error structure}
		\item Key: $\varepsilon_{ij}$ are independent across products
		\item This means: your taste shock for BMW is unrelated to your taste shock for Mercedes
		\item \textbf{Implication:} Model doesn't ``know'' that BMW and Mercedes are similar
		\item When BMW's price rises:
		\begin{wideitemize}
			\vspace{5pt}
			\item Logit: BMW consumers spread evenly across ALL other products
			\item Reality: BMW consumers mostly switch to Mercedes, Audi
		\end{wideitemize}
		\item We'll see how to fix this with demographics (Lecture 3) and nested logit
	\end{wideitemize}
\end{frame}
...
\begin{frame}{Case study: Uber surge pricing}
	\begin{wideitemize}
		...
		\item \textbf{Why is surge pricing endogenous?}
...
\begin{frame}{From estimation to policy}
	\begin{wideitemize}
		\item We've seen how to estimate demand with logit
		\item \textbf{Why do we care?}
		\item Demand estimates let us:
		\begin{wideenumerate}
			\vspace{5pt}
			\item Compute \textbf{elasticities} (how sensitive are consumers?)
			\item Compute \textbf{consumer welfare} (how much do consumers benefit?)
			\item Simulate \textbf{counterfactuals} (what if a product is removed? what if firms merge?)
		\end{wideenumerate}
		\item Next: demographics to relax IIA, then these applications
	\end{wideitemize}
\end{frame}
